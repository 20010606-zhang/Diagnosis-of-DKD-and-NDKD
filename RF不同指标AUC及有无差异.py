...
# 要绘制AUC曲线的特征数量
selected_feature_counts = [4, 7, 8, 17]

# 存储预测概率和AUC值
y_preds = []
aucs = []

plt.figure(figsize=(10, 8))  # 调整画布大小与之前代码一致

# 自定义图例名称
legend_names = ["RF models with 4 Features", "RF models with 7 Features", "RF models with 8 Features", "RF models with 17 Features"]
...
for i in range(len(selected_feature_counts) - 1):
    ...
    # 统一文本字体大小为12
    plt.text(0.55, 0.3 - i * 0.05,
             f'ΔAUC ({legend_names[i]} vs {legend_names[-1]}): {delta_auc:.2f}, p={p_value:.3f}',
             fontsize=8)

# 调整坐标轴范围和标签字体大小（与之前代码一致）
plt.xlim([0.0, 1.0])
plt.ylim([0.0, 1.05])
...
plt.savefig("ROC不同指标及AUC有无差异.png", format='png', bbox_inches='tight', dpi=300)
plt.tight_layout()
plt.show()
